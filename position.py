...
class Position(object):
    ...
    def __getitem__(self, item):
        return self.price_series[item]

    def __len__(self):
        return len(self.price_series)
    ...
    def last_value(self) -> Dollars:
        return self.last_price * self.units
    ...
    def value_series(self) -> pd.Series:
        ...
        return self.units * self.price_series[:-1]
    ...
    def exit_value(self) -> Dollars:
        return self.units * self.exit_price
    ...
    def print_position_summary(self) -> object:
        _entry_date = _pdate(self.entry_date)
        _exit_date = _pdate(self.exit_date)
        _days = self.trade_length

        _entry_price = round(self.entry_price, 6)
        _exit_price = round(self.exit_price, 6)

        _entry_value = round(self.entry_value, 6)
        _exit_value = round(self.exit_value, 6)

        _return = round(100 * self.percent_return, 2)
